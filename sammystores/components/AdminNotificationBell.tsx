...
// Admin-facing: polls all tickets for ones needing admin attention
// (adminUnread). Separate from the customer NotificationBell since it
// hits a different endpoint (/api/admin/tickets, verifyAdmin) and field.
export default function AdminNotificationBell() {
  const router = useRouter();
  const [tickets, setTickets] = useState<AdminNotifTicket[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.tickets)) {
        setTickets(data.tickets.filter((t: any) => t.adminUnread));
      }
    } catch {
  ...
  const ownerLabel = (t: AdminNotifTicket) => {
    if (typeof t.userId === 'string') return t.userId;
    return t.userId?.name || t.userId?.email || 'Customer';
  };
    ...
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-96 overflow-y-auto">
          {tickets.length === 0 ? (
            <p className="text-sm text-gray-500 p-4 text-center">No new tickets</p>
          ) : (
            tickets.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 truncate mt-0.5">{ownerLabel(t)}</p>
                  {lastMsg && (

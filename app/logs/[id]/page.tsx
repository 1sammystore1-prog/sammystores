...
export default function LogDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        const detailRes = await fetch(`/api/logs/products/${productId}`);
  ...
  const handleBuy = async () => {
    ...
    try {
      const res = await fetch('/api/logs/buy', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex flex-col md:flex-row max-w-7xl mx-auto">
          ...
          <main className="flex-1 p-6 md:p-8">
            ...
            <Link href="/logs" className="text-[#f97316] font-semibold">← Back to Buy Logs</Link>
          </main>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col md:flex-row max-w-7xl mx-auto">
        <Sidebar />
        <main className="flex-1 p-6 md:p-8">
          <Link href="/logs" className="text-sm text-gray-500 hover:text-[#f97316] mb-4 inline-block">
            ← Back to Buy Logs
          </Link>

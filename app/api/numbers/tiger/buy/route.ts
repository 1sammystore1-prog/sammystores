...
import { buyNumber } from '@/lib/tigerSms';
...
export async function POST(request: Request) {
  ...
  try {
    // Get live price first for accurate deduction
    const services = await fetch(`${process.env.TIGER_SMS_BASE_URL}/getNumbersStatus?api_key=${process.env.TIGER_SMS_API_KEY}&country=${country}`)
      .then(r => r.json())
      .then(d => d.find((s: any) => s.service === service));
    
    if (!services) return NextResponse.json({ error: 'Service unavailable' }, { status: 400 });
    
    const priceNgn = parseFloat(services.price) * 1550; // USD to NGN

...
import { cancelNumber } from '@/lib/benotp';
...
export async function POST(request: Request) {
  ...
  if (!pool) {
    return NextResponse.json({ error: 'Missing pool information for this order' }, { status: 500 });
  }

  try {
    await cancelNumber(pool, String(activationId));
  } catch (e: any) {

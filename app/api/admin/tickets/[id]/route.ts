...
// do both in one request instead of two. Same underlying logic as the
// Telegram reply webhook (app/api/webhooks/telegram/route.ts) - both
// call lib/ticketReply.ts so replying from either surface behaves
// identically.
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ticket = await replyToTicketAsAdmin(id, message, status);
  if (!ticket) {
    return NextResponse.json({ error: 'Ticket not found' }, { status: 404 });
  }
